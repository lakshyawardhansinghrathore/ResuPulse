...
def _call_groq(client: Groq, system_prompt: str, user_prompt: str) -> str:
    ...
    for model in CANDIDATE_MODELS:
        ...
        try:
            ...
            last_err = e
            logger.warning(f"Groq model {model} failed ({e}), trying fallback model...")
            continue
    raise last_err
...
def _validate_jd_result(result: dict) -> dict:
    """Ensure the parsed job description dictionary contains all expected schema fields."""
...
def _validate_resume_result(result: dict) -> dict:
    """Ensure the parsed resume dictionary contains all expected schema fields."""
